...
import { getCurrentUser , createMatch, deleteMatch, getSpecificMentee, getMentees} from "../../actions";
...
state ={
 ...
 async componentDidMount() {
    ...
    this.setState({...this.state, user: this.props.user, mentees:this.props.mentees})
    ...
    this.setState({...this.state, connect_id:this.state.mentees.filter(id => {return id.user_id === this.state.user.id})})
...
render() {
    return(
        <div>
            <h2>{this.props.id}</h2>
            <Button onClick={(e) => {this.apply(e)}}>Apply to This Mentor</Button>
            <Button onClick={(e) => {this.deleteApplication(e)}}>Remove Application</Button>
        </div>
    )
 }
}
